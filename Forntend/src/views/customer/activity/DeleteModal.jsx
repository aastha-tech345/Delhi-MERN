...
const DeleteModal = ({ setHide, activityId, getData }) => {
  // console.log('delete', activityId)
  const apiUrl = process.env.REACT_APP_API_URL
  const handleDeleteConfirm = async () => {
    if (activityId) {
      try {
        const response = await deleteFetch(`${apiUrl}/activity/get_activity`, activityId)
  ...
  return (
    <>
      <Modal show={true} centered>
        <Modal.Title>
          <svg
            style={{ marginLeft: '50px', marginTop: '25px', marginBottom: '15px' }}
            width="44"
            height="53"
            viewBox="0 0 44 53"
            fill="none"
          ...
          <br />
          <h4 style={{ color: 'black' }}>Sind Sie sicher?</h4>
        </Modal.Title>
        <Modal.Body>
